...
{
    md_addr_t repl_addr = 0; // STW
  if (cache_dl2)
    {
      /* access next level of data cache hierarchy */
      int access_time =  cache_access(cache_dl2, cmd, baddr, NULL, bsize,
              /* now */now, /* pudata */NULL, &repl_addr); // STW
      if (repl_addr != 0){ // STW
          if (cache_probe(cache_dl1, repl_addr) != 0){ // STW
            access_time += cache_flush_addr(cache_dl1, repl_addr, now); // STW
          }
      }
      return access_time;
    }
          ...
          int argc, char **argv)	/* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	victim_cache = cache_create("victim", 1 ,bsize, FALSE, 0, 4, cache_char2policy('l'),
				dl1_access_fn, 1);
